...
const RequestsComponent: React.FC<AddFriendProps> = ({ socket, }) => {
  ...
  useEffect(() => {
    ...
    if (user && user?.id && socket) {
      socket.emit("joinRoom", "friend_requests");
      
      socket.on(user.id, (data: ComingRequestsModel) => {
        console.log("arkadaslik geldi", data);
        ...
        toast(data.user_name + " arkadaslik istegi geldi");
      });
    }
  }, [socket]);
