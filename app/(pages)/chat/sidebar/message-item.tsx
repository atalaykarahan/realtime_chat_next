...
const MessageItem: React.FC<MessageItemProps> = ({ message }) => {
  const maxIdMessage = message.messages.reduce((prev, current) => {
    ...
    <div onClick={openMessageBox}  className="gap-4 py-2 lg:py-2.5 px-3 border-l-2 border-transparent hover:bg-default-200 cursor-pointer flex">
        ...
        <div className="block">
          ...
          <div className="truncate  max-w-[120px]">
            <span className=" text-xs  text-[#5C6B81] ">
              {/* 52 karakter  */}
              {maxIdMessage.message_content.length >= 52
                ? maxIdMessage.message_content.substring(0, 52) + "..."
                : maxIdMessage.message_content}
            </span>
          </div>
        </div>
      </div>
      <div className="flex-none  flex-col items-end  gap-2 hidden lg:flex">
        <span className="text-xs text-white text-end uppercase ">
          {new Date(maxIdMessage.createdAt).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
